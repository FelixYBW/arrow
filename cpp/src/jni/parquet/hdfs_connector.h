...
class HdfsConnector : public Connector {
 public:
  ...
  /// \param[hdfs_path] hdfs file url, hdfs:// is expected at the beginning of the string
  explicit HdfsConnector(std::string hdfs_path);
  ~HdfsConnector();

  /// \brief Open hdfs parquet file as readable handler
  /// \param[use_hdfs3] an option to indicate if libhdfs3 or libhdfs should be expected
