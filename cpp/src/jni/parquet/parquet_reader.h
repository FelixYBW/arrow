...
class ParquetReader {
 public:
  ...
  /// \param[path] ParquetReader will open difference connector according to file path
  explicit ParquetReader(std::string path);

  ~ParquetReader();

  /// \brief Initialization of ParquetReader
  /// \param[column_indices] indexes of columns expected to be read
  /// \param[row_group_indices] indexes of row_groups expected to be read
  ...
  /// \brief Initialization of ParquetReader
  /// \param[column_indices] indexes of columns expected to be read
  /// \param[start_pos] use offset to indicate which row_group is expected
  /// \param[end_pos] use offset to indicate which row_group is expected
  /// \param[batch_size] batch size, default is 4096
  /// \param[use_hdfs3] option used by HdfsConnector
  ...
  /// \param[out] readed batch will be returned as RecordBatch
 ...
 private:
  ...
  /// \brief calculate RowGroup indices through offset
  std::vector<int> GetRowGroupIndices(int num_row_groups, int64_t start_pos,
                                      int64_t end_pos);

  /// \brief get recordBatch reader
